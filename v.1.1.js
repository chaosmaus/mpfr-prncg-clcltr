$(document).ready(function () {
    ...
    const darkModeToggleOff = () => {
        ...
        if (toggle && $(`.toggle-active`).length === 0 && currentSegmentIndex <= 9) {
            ...
            $(`.no-price_line`).css(`background-color`, `#c3cdc8`)
            if ($(window).width() > 1439) {
                $(`.pricing-result_content-wrapper`).animate({ 'right': '-=125%' }, '500', 'easeInOutQuad');
            } else {
    ...
    const darkModeToggleOn = () => {
        if (!toggle) {
            ...
            $(`.no-price_line`).css(`background-color`, `rgba(0,0,0,0)`)
            if ($(window).width() > 1439) {
                $(`.pricing-result_content-wrapper`).animate({ 'right': '+=125%' }, '500', 'easeInOutQuad');
            }else{
                $(`.pricing-result_content-wrapper`).animate({ 'right': '+=140%' }, '500', 'easeInOutQuad');
